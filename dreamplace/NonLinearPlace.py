    ...
    def __call__(self, params, placedb, learning_rate_value):
        ...
        if params.global_place_flag:
            ...
            for cur_stage, global_place_params in enumerate(global_place_stages):
                ...
                def refine_with_pso(init_tensor):
                    if not getattr(params, "gift_pso_flag", 0):
                        return init_tensor
                    ...
                    swarm_size = max(int(getattr(params, "gift_pso_swarm", 8)), 1)
                    max_iters = max(int(getattr(params, "gift_pso_iters", 10)), 1)
                    inertia = float(getattr(params, "gift_pso_inertia", 0.6))
                    cognitive = float(getattr(params, "gift_pso_cognitive", 1.2))
                    social = float(getattr(params, "gift_pso_social", 1.2))
                    init_sigma = float(getattr(params, "gift_pso_init_sigma", 0.01))

                    xl, xh = placedb.xl, placedb.xh
                    yl, yh = placedb.yl, placedb.yh

                    base_flat = self.pos[0].clone().detach()
                    x_idx = torch.arange(
                        num_movable, device=device, dtype=torch.long
                    )
                    y_idx = torch.arange(
                        base_flat.numel() // 2,
                        base_flat.numel() // 2 + num_movable,
                        device=device,
                        dtype=torch.long,
                    )

                    base_particle = init_tensor[:, : placedb.num_physical_nodes].clone()
                    particle_core = base_particle[:, :num_movable]
                    swarm = particle_core.unsqueeze(0).repeat(swarm_size, 1, 1)
                    if init_sigma > 0:
                        swarm += init_sigma * (torch.rand_like(swarm) - 0.5)
                    velocities = torch.zeros_like(swarm)
                    personal_best = swarm.clone()

                    def evaluate(particle_xy):
                        candidate = base_flat.clone()
                        candidate[x_idx] = particle_xy[0]
                        candidate[y_idx] = particle_xy[1]
                        with torch.no_grad():
                            cost = model.obj_fn(candidate)
                        return float(cost)

                    personal_scores = torch.empty(swarm_size, device=device)
                    for idx in range(swarm_size):
                        personal_scores[idx] = evaluate(swarm[idx])
                    global_best_idx = int(torch.argmin(personal_scores).item())
                    global_best = personal_best[global_best_idx].clone()

                    for _ in range(max_iters):
                        r1 = torch.rand_like(swarm)
                        r2 = torch.rand_like(swarm)
                        velocities = (
                            inertia * velocities
                            + cognitive * r1 * (personal_best - swarm)
                            + social * r2 * (global_best.unsqueeze(0) - swarm)
                        )
                        swarm = swarm + velocities
                        swarm[:, 0, :].clamp_(xl, xh)
                        swarm[:, 1, :].clamp_(yl, yh)

                        for idx in range(swarm_size):
                            score = evaluate(swarm[idx])
                            if score < personal_scores[idx]:
                                personal_scores[idx] = score
                                personal_best[idx] = swarm[idx].clone()
                        global_best_idx = int(torch.argmin(personal_scores).item())
                        global_best = personal_best[global_best_idx].clone()

                    refined = init_tensor.clone()
                    refined[0, :num_movable] = global_best[0]
                    refined[1, :num_movable] = global_best[1]
                    return refined
