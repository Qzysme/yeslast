...
class PlaceObj(nn.Module):
    ...
    def __init__(self, density_weight, params, placedb, data_collections,
        ...
        # pin risk map for pin-aware objective and GiFtPlus sampling
        pin_risk_map = self.build_pin_risk_map(placedb, num_bins_x, num_bins_y)
        risk_tensor = torch.from_numpy(pin_risk_map).to(
            dtype=self.data_collections.pos[0].dtype,
            device=self.data_collections.pos[0].device,
        )
        risk_tensor = risk_tensor.unsqueeze(0).unsqueeze(0)
        self.register_buffer("pin_risk_tensor", risk_tensor)
        self.pin_risk_map_np = pin_risk_map
        self.pin_risk_weight = float(getattr(params, "pin_risk_weight", 0.0))

        # GiFt initialization
        self.op_collections.gift_init_op = None
        if params.global_place_flag:
            if getattr(params, "giftplus_init_flag", 0):
                self.op_collections.gift_init_op = gift_init.GiFtPlusInit(
                    ...
                    pin_risk_map=self.pin_risk_map_np,
                    sparsify_k=getattr(params, "giftplus_sparsify_k", 16),
                    dirichlet_iters=getattr(params, "giftplus_dirichlet_iters", 3),
                    refine_iters=getattr(params, "giftplus_refine_iters", 2),
                    adapt_flag=getattr(params, "gift_adapt_flag", 0),
                    adapt_samples=getattr(params, "gift_adapt_samples", 32),
                    adapt_lambda_hpwl=getattr(params, "gift_adapt_lambda_hpwl", 1.0),
                    adapt_lambda_density=getattr(params, "gift_adapt_lambda_density", 0.2),
                    adapt_lambda_risk=getattr(params, "gift_adapt_lambda_risk", 0.3),
                    adapt_risk_step=getattr(params, "gift_adapt_risk_step", 5.0),
                    adapt_seed=getattr(params, "gift_adapt_seed", 2025),
                )
            elif params.gift_init_flag:
    ...
    def obj_fn(self, pos):
        ...
        if self.pin_risk_weight != 0.0:
            self.pin_risk_penalty = self.compute_pin_risk_penalty(pos)
            result = result + self.pin_risk_weight * self.pin_risk_penalty

        return result

    def build_pin_risk_map(self, placedb, num_bins_x, num_bins_y):
        bins_x = max(int(num_bins_x), 1)
        bins_y = max(int(num_bins_y), 1)
        bin_size_x = (placedb.xh - placedb.xl) / bins_x
        bin_size_y = (placedb.yh - placedb.yl) / bins_y
        bin_area = max(bin_size_x * bin_size_y, 1e-12)
        risk_map = np.zeros((bins_y, bins_x), dtype=np.float32)

        if placedb.num_physical_nodes == 0:
            return risk_map

        if placedb.flat_node2pin_start_map is not None:
            start_map = placedb.flat_node2pin_start_map
            physical_limit = min(len(start_map) - 1, placedb.num_physical_nodes)
            node_pin_counts = start_map[1 : physical_limit + 1] - start_map[:physical_limit]
            if node_pin_counts.size < placedb.num_physical_nodes:
                pad = placedb.num_physical_nodes - node_pin_counts.size
                node_pin_counts = np.pad(
                    node_pin_counts, (0, pad), mode="constant", constant_values=0
                )
        else:
            node_pin_counts = np.zeros(placedb.num_physical_nodes, dtype=np.int32)
        ...
        for node_id in range(placedb.num_movable_nodes, placedb.num_physical_nodes):
            width = placedb.node_size_x[node_id]
            height = placedb.node_size_y[node_id]
            if width <= 0 or height <= 0:
                continue
            x = placedb.node_x[node_id]
            y = placedb.node_y[node_id]
            x1 = max(x, xl)
            y1 = max(y, yl)
            x2 = min(x + width, xh)
            y2 = min(y + height, yh)
            ...
            ix_begin = max(int((x1 - xl) / bin_size_x), 0)
            ix_end = min(int(np.ceil((x2 - xl) / bin_size_x)), bins_x)
            iy_begin = max(int((y1 - yl) / bin_size_y), 0)
            iy_end = min(int(np.ceil((y2 - yl) / bin_size_y)), bins_y)
            pin_weight = 1.0 + float(node_pin_counts[node_id])

            for by in range(iy_begin, iy_end):
                bin_yl = yl + by * bin_size_y
                bin_yh = bin_yl + bin_size_y
                overlap_y = min(y2, bin_yh) - max(y1, bin_yl)
                if overlap_y <= 0:
                    continue
                for bx in range(ix_begin, ix_end):
                    bin_xl = xl + bx * bin_size_x
                    bin_xh = bin_xl + bin_size_x
                    overlap_x = min(x2, bin_xh) - max(x1, bin_xl)
                    if overlap_x <= 0:
                        continue
                    overlap_area = overlap_x * overlap_y
                    risk_map[by, bx] += pin_weight * overlap_area / bin_area

        if np.any(risk_map):
            try:
                from scipy.ndimage import gaussian_filter

                sigma = max(min(num_bins_x, num_bins_y) * 0.03, 1.0)
                risk_map = gaussian_filter(risk_map, sigma=sigma, mode="nearest")
            except Exception:
                # simple smoothing fallback
                kernel = np.array([[1, 2, 1], [2, 4, 2], [1, 2, 1]], dtype=np.float32)
                kernel /= kernel.sum()
                padded = np.pad(risk_map, 1, mode="edge")
                smoothed = np.zeros_like(risk_map)
                for i in range(risk_map.shape[0]):
                    for j in range(risk_map.shape[1]):
                        window = padded[i : i + 3, j : j + 3]
                        smoothed[i, j] = np.sum(window * kernel)
                risk_map = smoothed
            max_val = np.max(risk_map)
            if max_val > 0:
                risk_map = risk_map / max_val
        return risk_map

    def compute_pin_risk_penalty(self, pos):
        if not hasattr(self, "pin_risk_tensor") or self.pin_risk_tensor is None:
            return torch.zeros([], dtype=pos.dtype, device=pos.device)
        if self.placedb.num_movable_nodes <= 0:
            return torch.zeros([], dtype=pos.dtype, device=pos.device)
        pos2d = pos.view(2, -1)
        movable_pos = pos2d[:, : self.placedb.num_movable_nodes]
        norm_x = (movable_pos[0] - self.placedb.xl) / max(
            self.placedb.xh - self.placedb.xl, 1e-12
        )
        norm_y = (movable_pos[1] - self.placedb.yl) / max(
            self.placedb.yh - self.placedb.yl, 1e-12
        )
        grid_x = norm_x * 2 - 1
        grid_y = norm_y * 2 - 1
        grid = torch.stack([grid_x, grid_y], dim=1)
        grid = grid.view(1, -1, 1, 2)
        risk_tensor = self.pin_risk_tensor
        if risk_tensor.device != pos.device or risk_tensor.dtype != pos.dtype:
            risk_tensor = risk_tensor.to(device=pos.device, dtype=pos.dtype)
        ...
        return torch.sum(sampled * node_areas)
